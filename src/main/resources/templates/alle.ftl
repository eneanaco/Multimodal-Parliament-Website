...
<div class="container">
    ...
    <!-- Buttons to decide the typ of export-format -->
    <div class="feature-card">
        <h2>XMI</h2>
        <button id="xmi-export" class="feature-button">Export aller Reden in XMI</button>
        <h2>PDF</h2>
        <button id="pdf-export" class="feature-button">Export aller Reden in PDF</button>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#xmi-export').click(function() {
            $.ajax({
                type: 'POST',
                url: '/alle/xmi',
                success: function(data) {
                    console.log('XMI-Export erfolgreich');
                },
                error: function(xhr, status, error) {
                    console.error('Fehler beim XMI-Export:', error);
                }
            });
        });
